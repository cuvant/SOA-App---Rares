...
(function() {
  ...
  (function(alerts, $) {
    alerts.confirm_token = function(alert_id, widget_id) {
      var token;
      ...
      if ($("#text_message_confirmation_" + alert_id).data("started") !== true) {
        ...
        return $.ajax({
          ...
          error: function(jqXHR, textStatus, errorThrown) {
            ...
            return $("#text_message_confirmation_" + alert_id).data('started', false);
          }
        });
      }
    };
    alerts.successfully_confirmed = function(alert_id, widget_id) {
      ...
      if ($(".pending_verification").length === 0) {
        ...
        return $("#pending-alerts-settings").remove();
      }
    };
    alerts.add_spinner = function(alert_id) {
      var spinner;
      spinner = document.createElement("I");
      spinner.classList.add("fa", "fa-spinner", "fa-spin");
      return $("#errors_" + alert_id).html(spinner);
    };
    alerts.check_token = function(input, alert_id) {
      var token;
      $("#errors_" + alert_id).html("");
      token = String($(input).val());
      if (token.match(/^\d{4,4}$/)) {
        return $(input).css('color', 'green');
      } else {
        return $(input).css('color', 'red');
      }
    };
    return alerts.resend_token = function(alert_id, widget_id) {
      return $.ajax({
        type: 'get',
        data: {},
        ...
        dataType: 'json',
        success: function(data, textStatus, jqXHR) {
          return $("#errors_" + alert_id).html("<p style='color:green; padding-top:7px;'>" + data.text + "</p>");
        },
        error: function(jqXHR, textStatus, errorThrown) {
          return $("#errors_" + alert_id).html("<p style='color:red; padding-top:7px;'>" + jqXHR.responseText + "</p>");
        }
      });
    };
  })(window.dashboards.alerts = window.dashboards.alerts || {}, jQuery);
...
(function() {
  ...
  (function(cocoon, $) {
    return cocoon.init_callback = function() {
      return $('.alerts').on('cocoon:after-insert', function() {
        $("#widget_form").enableClientSideValidations();
      });
    };
  })(window.dashboards.cocoon = window.dashboards.cocoon || {}, jQuery);
...
(function() {
  ...
  (function(shared, $) {
    shared.init = function(scrollTo) {
      return shared.initFlashNotice(5);
    };
    shared.initFlashNotice = function(display_for_seconds) {
      if ($('.flash-container').length !== 0) {
        setTimeout(function() {
          return $('.flash-container').slideUp();
        });
      }
      return $(document).on('click', '.close', function() {
        return $('.flash-container').hide();
      });
    };
    return $(document).ready(function() {
      return $('.flash-container').fadeOut(7000);
    });
  })(window.dashboards.shared = window.dashboards.shared || {}, jQuery);
...
(function() {
  ...
  (function(widgets, $) {
    return widgets.set_form = function() {
      return $('input[name="widget_type_radio"]').on('ifChecked', function(event) {
        var type;
        ...
        return $.ajax({
          ...
          data: {
            dashboard_id: window.dashboard_id
          },
          dataType: 'script'
        });
      });
    };
  })(window.dashboards.widgets = window.dashboards.widgets || {}, jQuery);
...
ready = function() {
    ...
    $('.table-responsive').off('show.bs.dropdown')
    $('.table-responsive').on('show.bs.dropdown', function () {
         $('.table-responsive').css( "overflow", "inherit" );
    });

    $('.table-responsive').off('hide.bs.dropdown')
